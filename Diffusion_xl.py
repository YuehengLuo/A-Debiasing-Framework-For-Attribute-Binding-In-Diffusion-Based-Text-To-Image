...
class DiffusionXLPipeline(
    ...
    """
    def prepare_candidates(self, offline_file=None, save_path=None, obj_file="your path of candidates.txt"):
        ...
        # print("Candidates loaded:", self.candidates)
        # print("Candidate embeddings shape:", self.candidate_embs.shape)
    ...
    def get_magnet_direction(
        ...
        if not hasattr(self, "candidates") or self.candidates is None:
            print("Warning: candidates not set. Skipping magnet direction computation.")
            return
        ...
        for pid, pair in enumerate(pairs):
            try:
                ...
                # print(f"Current concept index: {cur_concept_index}")
                ...
                # print("pull_direction", pull_direction)
                ...
                for outid, outpair in enumerate(pairs):
                    ...
                    negative_embeds =  [self.get_eot(neg, -1) for neg in negative]
                    push_direction = [negative_embed - uncond_embed for uncond_embed, negative_embed in zip(uncond_embeds, negative_embeds)]
                ...
                print(f"Error processing pair {pair} in prompt '{prompt}': {e}")
                raise e
    ...
    def __call__(
        ...
        Alternating_prompts: Union[str, List[str]] = None,
        seed: Optional[int] = None,
        prompt_2: Optional[Union[str, List[str]]] = None,
        height: Optional[int] = None,
        width: Optional[int] = None,
        num_inference_steps: int = 50,
        Bias_level_to_transition_step: List[int] = [0, 3, 5, 10, 20, 30],
        alt_step: int = 2,
        ...
        **kwargs,
        ...
        """
        self.Alternating_prompts = Alternating_prompts
        self.h = height
        self.w = width
        self.pn = True
        self.eq = True
        ...
        Bias_level = Alternating_prompts["Bias_level"]
        transition_steps = [Bias_level_to_transition_step[int(level)] for level in Bias_level] + [num_inference_steps]

        Alternating_prompts = Alternating_prompts["Alternating_prompt"][0]
        ...
        # Get all embeddings for Alternating_prompts
        prompt_embeds_list = []
        add_text_embeds_list = []
        add_time_ids_list = []
        timestep_cond_list = []
        for i, prompt in enumerate(Alternating_prompts):
            (
                prompt_embeds,
                negative_prompt_embeds,
                pooled_prompt_embeds,
            ...
            with torch.no_grad():
                self.get_magnet_direction(prompt, alpha_lambda=0.6, K=5, neighbor="feature")
